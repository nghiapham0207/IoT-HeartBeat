...
#     x = [[61, 1, 0, 138, 166, 125]]
#     randomforest = pickle.load(open('model.sav', 'rb'))
#     predictions = randomforest.predict(x)
#     if predictions==0:
#         # myText.set('********Less chance of Heart Stroke********')
#         print("Khả năng đột quỵ thấp")
#     else:
#         # myText.set('********High chance of Heart Stroke********')
#         print("Khả năng đột quỵ cao")

def prediction_model(age, sex, cp, huyetap, chol, maxBMP):

    # x = [[int(entry_1.get()), int(entry_2.get()), int(entry_3.get()), int(entry_4.get()), int(entry_5.get()), int(entry_6.get()), int(entry_7.get()), int(entry_8.get()), int(entry_9.get()), float(entry_10.get()), int(entry_11.get()), int(entry_12.get()), int(entry_13.get())]]
    x = [[age, sex, cp, huyetap, chol, maxBMP]]
    # x = [[int(age), int(sex), int(cp), int(huyetap), int(chol), int(maxBMP) ]]
    randomforest = pickle.load(open(
        'C:\\Users\\nghia\\Downloads\\Nhom2_Theo-doi-nhip-tim-firebase-website\\python\\iot\\Heart-Stroke-Prediction-using-ML\\model.sav', 'rb'))
    predictions = randomforest.predict(x)
    if predictions[0] == 0:
        # myText.set('********Less chance of Heart Stroke********')
        # print("Khả năng đột quỵ thấp")
        return 'Khả năng đột quỵ thấp'
    else:
        # myText.set('********High chance of Heart Stroke********')
        # print("Khả năng đột quỵ cao")
        return "Khả năng đột quỵ cao"


# df=pd.read_csv('heart.csv')
#
# predictors = df.drop(['target', 'exang', 'oldpeak', 'slope', 'ca', 'thal', 'restecg', 'fbs'],  axis=1)
# print(predictors)
# target = df["target"]
